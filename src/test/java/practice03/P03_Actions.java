...
/*
renk degisimini boyle alabilirsiniz

akabinde ikisinin ayni olmadigini assert edebiliriz
Actions actions = new Actions(driver);
WebElement blueBox = driver.findElement(By.id("div2"));
String firstColor = blueBox.getCssValue("background-color"); //ilk renk (mavi)
System.out.println(firstColor);  //rgba(0, 0, 255, 1)

actions.moveToElement(blueBox).perform();
waitFor(3);
String secondColor = blueBox.getCssValue("background-color"); //son renk (yesil)
System.out.println(secondColor);  //rgba(0, 128, 0, 1)
 */